...
下列为**判断框架 / 人物视角**，各自擅长的问题域不同，并行打分后由 Team Lead 综合，不是互相替代。
...
**另维：资金面（不是人物视角）**  
`a-share-capital-flow` 看的是参与者资金行为（主力净流入估算、北向、两融、龙虎榜等），用来验证「价格运动有没有资金配合」。它与上表技术框架 / 投资人视角**不在同一维度**：上表回答「怎么判断」，资金面回答「谁在买、钱往哪流」。深研时按需注入，不与段永平/巴菲特等并列成「某位大师」。

研报与财报 JSON 同理属于**证据取数层**（供巴菲特 / 段永平视角引用），不是独立「第五位」判断人格。
...
自选股清单与勾选状态均保存在本机 SQLite（`data/portfolio.db`），不依赖浏览器；换浏览器仍保留。旧版浏览器 `localStorage` 勾选会在首次打开时自动迁移到本机库。
