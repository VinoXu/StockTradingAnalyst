...
| 字段 | 口径 | 推断用途 |
|------|------|----------|
| `main_side_net` | 超大单+大单净流入（主力代理） | 大单侧方向 |
| `retail_proxy_net` | 中单+小单净流入（散户代理） | 散户侧方向 |
| `behavior_tags` | 价量+大小单组合标签 | 吸筹/派发/接盘/FOMO 等人性模式 |
| `participant_flow.northbound` | 全市场北向净买 | 外资态度 |
| `lhb_institution` | 龙虎榜机构席买卖 | 仅上榜股、上榜日 |

| 行为组合 | 可能意图（须写证伪条件） |
|----------|------------------------|
| 价涨 + 大单入 + 小单出 | 推升吸筹，散户止盈 |
| 价涨 + 大单出 + 小单入 | 派发，散户接盘（FOMO） |
| 价跌 + 大单入 + 小单出 | 下跌承接，清洗浮筹 |
| 北向净买 + 内资大单出 | 内外资分歧 |
| 融资急增 + 价滞涨 | 杠杆散户过热 |
| 机构席净买 + 游资席净卖 | 配置 vs 短线兑现 |

**禁止**把 `behavior_tags` 当事实；必须写「东财估算/上榜日明细」口径。
